import type { Item, Effects, Stuff, Set, ItemCount } from "../types.ts";
import { items } from "../items/index.ts.ts";
import { sets } from "../sets/index.ts";

function calculateEffectContribution(item: Item, requiredEffects: Effects): number {
	let contribution = 0;
...
function findBestItems(requiredEffects: Effects, baseEffects: Effects): Item[] {
	...
	items.sort(
		(a, b) => calculateEffectContribution(b, requiredEffects) - calculateEffectContribution(a, requiredEffects)
	);
	...
	for (const item of items) {
		...
		if (
			Object.keys(requiredEffects).every(
				(effect) =>
					(currentEffects[effect as keyof Effects] ?? 0) >= (requiredEffects[effect as keyof Effects] ?? 0)
			)
		) {
			break;
		}
		...
		for (const effect in item.effects) {
			...
			currentEffects[key] = (currentEffects[key] || 0) + (item.effects[key] || 0);
		}
	}
...
export function greedy(minRequiredEffects: Effects, baseEffects: Effects): { stuff: Stuff; combinedStats: Effects } {
	...
	const stuff: Stuff = {
		items: {
			...
			petSlot: bestItems.find((item) => item.category_id === 22) || ({} as Item),
			weaponSlot: bestItems.find((item) => item.category_id === 1) || ({} as Item),
			DofusSlot1: bestItems.find((item) => item.category_id === 20) || ({} as Item),
			DofusSlot2: bestItems.find((item) => item.category_id === 20) || ({} as Item),
			DofusSlot3: bestItems.find((item) => item.category_id === 20) || ({} as Item),
			DofusSlot4: bestItems.find((item) => item.category_id === 20) || ({} as Item),
			DofusSlot5: bestItems.find((item) => item.category_id === 20) || ({} as Item),
			DofusSlot6: bestItems.find((item) => item.category_id === 20) || ({} as Item),
		},
	};
